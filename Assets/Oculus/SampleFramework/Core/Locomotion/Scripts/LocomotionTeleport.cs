...
public class LocomotionTeleport : MonoBehaviour
{
	...
	public bool AimCollisionTest(Vector3 start, Vector3 end, LayerMask aimCollisionLayerMask, out RaycastHit hitInfo)
	{
		...
		switch (AimCollisionType)
		{
			...
			{
				var res = Physics.Raycast(start, direction, out hitInfo, distance * 100, aimCollisionLayerMask,
					QueryTriggerInteraction.Ignore);
				Debug.Log($"Hit: {res}");
				Debug.Log($"Start: {start}");
				Debug.Log($"Direction: {direction}");
				Debug.Log($"Distance: {distance}");
				Debug.Log($"Hitted: {hitInfo.transform}");
				if (res)
				{
					Debug.Log($"Hitted: {hitInfo.transform}");
					Debug.Log($"Hitted: {hitInfo.transform.gameObject}");
					Debug.Log($"Hitted: {hitInfo.transform.gameObject.name}");
				}
